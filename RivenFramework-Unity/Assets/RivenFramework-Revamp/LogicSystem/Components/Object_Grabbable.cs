    ...
    public class Object_Grabbable : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            else
            {
                ...
                //if (Alt_Item_Geodesic_Utility_GeoGun.delayRiftCollapse == false && Vector3.Distance(gameObject.transform.position, targetPawn.physObjectAttachmentPoint.transform.position) > breakAwayDistance)
                if (Vector3.Distance(gameObject.transform.position, targetPawn.physObjectAttachmentPoint.transform.position) > breakAwayDistance)
                {
                    ToggleHeld();
                }
            }
