...
public class LogicOutputDrawer : EasyPropertyDrawer
{
    ...
    {
        if (property.Property.serializedObject.isEditingMultipleObjects)
        {
            contents.Add(new Label("<b>Cannot edit Logic Output with multiple objects selected</b>").UseRichText());
            Boxed boxedCannotEditMultiple = new Boxed(contents);
            boxedCannotEditMultiple.boldness = 2;
            return boxedCannotEditMultiple;
        }
